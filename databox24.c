...
char	*gAgent = "DataBox24_Jan-28-2021/v0.03";		// This is used on the HTTP POST call specifing the agent that is calling
...
char *envp;
{
	...
	if ( !rc )
	{
		...
		if ( wl == RCV_HELLO_LEN )
		{
			...
			sprintf( gDbgBuf, "DataBox: %04X | Inverter# %04X | TotalWatts=%f, value=%x", 
				Swap2Endian(RH->head.databoxid), 
				Swap4Endian(RH->head.inverterid), 
				SwapFloatEndian(RH->head.totalwatts) );
...
ProcessStatus( char *_status, int _inverterid )
{
	...
	/* -------------------------------------------------------------------- */
	sprintf( locbuf, "Cmd=%02X&DataBoxID=%04X&AreaID=%04X&InverterID=%08X&TotalWatts=%f&CheckDigit=%02X&Unknown=%04X&Status=%02X", 
		CRIS->head.cmd, 
